...
class WCMp_Gateway_Den extends WCMp_Payment_Gateway {
    ...
    public function __construct() {
        $this->id = 'den';
        $this->enabled = get_wcmp_vendor_settings('payment_method_den', 'payment');
        $this->key_id = get_wcmp_vendor_settings('key_id', 'payment', 'den');
        $this->key_secret = get_wcmp_vendor_settings('key_secret', 'payment', 'den');
        ...
        if ($this->validate_request()) {
            ...
            if ($paypal_response && isset($paypal_response['seccess']) && !empty($paypal_response['seccess'])) {
                ...
                if ($this->transaction_id) {
                    return array('message' => __('New transaction has been initiated', 'wcmp-den-checkout-gateway'), 'type' => 'success', 'transaction_id' => $this->transaction_id);
                }
            } else {
                return false;
            }
    ...
    public function validate_request() {
        ...
        if ($this->enabled != 'Enable') {
            $this->message[] = array('message' => __('Invalid payment method', 'wcmp-den-checkout-gateway'), 'type' => 'error');
            return false;
        } else if (!$this->key_id && !$this->key_secret) {
            $this->message[] = array('message' => __('den payout setting is not configured properly. Please contact site administrator', 'wcmp-den-checkout-gateway'), 'type' => 'error');
            return false;
        } else if (!$this->reciver_email) {
            $this->message[] = array('message' => __('Please update your den Account information to receive commission', 'wcmp-den-checkout-gateway'), 'type' => 'error');
            return false;
        }
        ...
        if ($this->transaction_mode != 'admin') {
            ...
            if ($this->get_transaction_total() > $thesold_amount) {
                ...
                $this->message[] = array('message' => __('Minimum thesold amount to withdrawal commission is ' . $thesold_amount, 'wcmp-den-checkout-gateway'), 'type' => 'error');
                return false;
            }
        }
     ...
     public function process_den_payout() {
        $response = array();
            ...
            foreach ($this->commissions as $commission_id) {
                ...
                if ($vendor_order_id) {
                    ...
                    if ($vendor_order) {
                        ...
                        $note = sprintf(__('Total commissions earned from %1$s as at %2$s on %3$s', 'wcmp-den-checkout-gateway'), get_bloginfo('name'), date('H:i:s'), date('d-m-Y'));
                        $acceptedOrderStatus = apply_filters('wcmp_den_payment_order_status', array('processing', 'on-hold', 'completed'));
                        //check payment mode
                        if ($paymentMode != 'den') {
                            //payment method is not valid
                            $commissionResponse['message'] = "Order is not processed With den!"
                            ...
                            try {
                                ...
                                //log gateway error
                                doProductVendorLOG('den Comission Payment Error!!'
